-- Enhanced security for bitrix_credentials table (fixed version)
-- This migration adds comprehensive security measures while preserving existing data
...
-- 3. Drop existing broad RLS policy and create more granular ones
...
-- 4. Create audit table RLS policies
...
-- 6. Add validation constraints (more lenient for portal URLs)
ALTER TABLE public.bitrix_credentials 
ADD CONSTRAINT check_user_id_not_null CHECK (user_id IS NOT NULL);

-- More lenient portal URL validation
ALTER TABLE public.bitrix_credentials 
ADD CONSTRAINT check_portal_url_valid CHECK (
  portal_url IS NOT NULL AND 
  portal_url ~ '^https?://[a-zA-Z0-9.-]+\.(bitrix24|b24)\.[a-zA-Z]{2,}(/.*)?$'
);
