...
// URL de redirect configurável via env
const CALLBACK_URL = Deno.env.get("BITRIX_REDIRECT_URL") || "";
...
const ALLOW_ORIGINS = new Set<string>((Deno.env.get("ALLOWED_ORIGINS") || "").split(/[\s,]+/).filter(Boolean));
...
serve(async (req) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET || !CALLBACK_URL) {
    return new Response(JSON.stringify({ 
      error: "Configuração incompleta: BITRIX_CLIENT_ID/SECRET ou BITRIX_REDIRECT_URL não configurados" 
    }), {
      status: 500,
      headers: { ...cors(origin), "Content-Type": "application/json" },
    });
